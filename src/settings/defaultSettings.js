...
/**
 * FALLBACK director instruction. The real default lives in prompts/Default.txt — initSettings()
 * loads it (with the other built-in profiles) and patches it over `defaultSettings.directorPrompt`
 * before the defaults are applied, so this short version is only ever used if that fetch fails.
 * {{user}} / {{char}} macros are resolved at request time.
 */
...
export const defaultSettings = {
	...
	// The built-in profiles from prompts/*.txt are seeded in here by initSettings().
	promptProfiles: [], // [{ name, prompt }]
	// Names of built-in prompts (prompts/*.txt) already seeded into promptProfiles. Each built-in is
	// seeded exactly once, so renaming or deleting it isn't undone by the next reload.
	seededPromptProfiles: [], // [name]
	...
	// How many of the director's own PREVIOUS outlines to show it (interleaved into the history,
	// each right before the bot message it directed), so new directions stay coherent with past
	// decisions instead of re-deciding the scene from scratch every turn. 0 = none.
	previousOutlines: 0,
